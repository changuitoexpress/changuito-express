...
import { ArrowLeft, Plus, Phone, RefreshCw, X, MapPin } from 'lucide-react';
...
export default function VentaAutos(props: Props) {
  ...
  async function subirFotos(archivos: File[]): Promise<string[]> {
    ...
    for (let i = 0; i < archivos.length; i++) {
      ...
      const ruta = 'autos/' + Date.now() + '_' + i + '.' + ext;
  ...
  async function publicar() {
    ...
    try {
      ...
      const extraParts = [
        ...
        form.km   ? '🛣 ' + parseInt(form.km).toLocaleString('es-MX') + ' km' : '',
        form.transmision ? '⚙ ' + (form.transmision === 'manual' ? 'Manual' : form.transmision === 'cvt' ? 'CVT' : 'Automático') : '',
      ].filter(Boolean);
  ...
  const filtered = items.filter(function(it) {
    return !search || it.titulo.toLowerCase().includes(search.toLowerCase()) || (it.descripcion ?? '').toLowerCase().includes(search.toLowerCase());
  });
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', color: 'var(--text-primary)', fontFamily: 'system-ui,sans-serif', maxWidth: '480px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <h1 style={{ fontSize: '18px', fontWeight: 900, color: 'var(--text-primary)', margin: 0 }}>Venta de Autos</h1>
            </div>
          </div>
      ...
      <div style={{ padding: '16px', paddingBottom: '100px' }}>
        ...
        {/* Banner */}
        <div style={{ padding: '18px', borderRadius: '18px', background: 'linear-gradient(135deg,#1a1a2e,#7c3aed)', marginBottom: '20px', position: 'relative', overflow: 'hidden' }}>
          ...
          <p style={{ fontSize: '16px', fontWeight: 900, color: '#fff', margin: '0 0 4px 0' }}>Venta de Autos</p>
          <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.7)', margin: 0 }}>Lomas 1, 2, 3 & La Vista · {items.length} vehículos</p>
        </div>
        ...
        {loading && [1,2,3].map(function(i) {
          return (
            <div key={i} style={{ borderRadius: '16px', padding: '16px', background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', marginBottom: '10px', display: 'flex', gap: '12px' }}>
              <div className="skeleton" style={{ width: '80px', height: '60px', borderRadius: '12px', flexShrink: 0 }} />
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '60px 24px' }}>
            ...
            <p style={{ fontSize: '16px', fontWeight: 800, color: 'var(--text-primary)', margin: '0 0 6px 0' }}>Sin autos publicados</p>
        ...
        {!loading && filtered.map(function(it) {
          return (
            <div key={it.id} style={{ borderRadius: '16px', background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', marginBottom: '12px', overflow: 'hidden', boxShadow: 'var(--shadow-card)' }}>
              {it.fotos && it.fotos[0] ? (
                <img src={it.fotos[0]} alt={it.titulo} style={{ width: '100%', height: '180px', objectFit: 'cover' }} />
              ) : (
                <div style={{ width: '100%', height: '120px', background: 'linear-gradient(135deg,#1a1a2e,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px' }}>
                  🚗
                </div>
              )}
              <div style={{ padding: '14px 16px' }}>
                ...
                {it.contacto_wa && (
                  <a href={'https://wa.me/' + it.contacto_wa.replace(/\D/g,'') + '?text=' + encodeURIComponent('Hola, vi tu auto en Changuito Express: ' + it.titulo)}
      ...
      {modalAbierto && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 300, display: 'flex', alignItems: 'flex-end' }}>
          ...
          <div style={{ position: 'relative', width: '100%', maxWidth: '480px', margin: '0 auto', background: 'var(--bg-card)', borderRadius: '24px 24px 0 0', padding: '20px 20px 40px', zIndex: 301, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <h2 style={{ fontSize: '18px', fontWeight: 900, color: 'var(--text-primary)', margin: '0 0 20px 0' }}>🚗 Publicar vehículo</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {[
                { label: 'Título *', key: 'titulo', ph: 'Ej: Honda Civic 2018 Automático' },
                { label: 'Descripción', key: 'descripcion', ph: 'Estado, equipamiento, etc.' },
                { label: 'Año', key: 'anio', ph: '2020' },
                { label: 'Kilometraje', key: 'km', ph: '45000' },
            ...
            <button onClick={publicar} disabled={guardando || !form.titulo.trim()}
              ...
              {guardando ? <span className="spinner" style={{ width: '16px', height: '16px', borderWidth: '2px' }} /> : '🚗 Publicar vehículo'}
            </button>
          </div>
        </div>
      )}
