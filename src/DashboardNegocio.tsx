...
export default function DashboardNegocio(props: Props) {
  ...
  const [modalProducto, setModalProd] = useState<Partial<Producto> | null>(
    null,
  );
  ...
  const fetchMerchant = useCallback(
    async function () {
      ...
      try {
        ...
        if (!merchantId) {
          setError(
            "Tu cuenta no está vinculada a ningún negocio. Contacta a Changuito Express.",
          );
          setLoading(false);
          return;
        }
  ...
  async function toggleAbierto() {
    ...
    await supabase
      .from("merchants")
      .update({ is_open: nuevo })
      .eq("id", merchant.id);
    setMerchant(function (prev) {
      return prev ? { ...prev, is_open: nuevo } : prev;
    });
  }
  ...
  async function toggleProducto(prod: Producto) {
    ...
    await supabase
      .from("products")
      .update({ is_available: nuevo })
      .eq("id", prod.id);
    setProductos(function (prev) {
      return prev.map(function (p) {
        return p.id === prod.id ? { ...p, is_available: nuevo } : p;
      });
    });
  }
  ...
  async function guardarProducto() {
    ...
    if (!modalProducto.name?.trim()) {
      setError("El nombre es obligatorio.");
      return;
    }
    if (!modalProducto.price || modalProducto.price <= 0) {
    ...
    try {
      if (modalProducto.id) {
        ...
        setProductos(function (prev) {
          return prev.map(function (p) {
            return p.id === modalProducto.id
              ? ({ ...p, ...modalProducto } as Producto)
              : p;
          });
        });
      } else {
  ...
  if (error && !merchant) {
    return (
      ...
      >
        ...
        <a
          href="https://wa.me/522223339999?text=Quiero%20acceso%20al%20dashboard%20de%20mi%20negocio"
          style={{
            padding: "12px 24px",
            borderRadius: "14px",
            background: "#25D366",
    ...
    >
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: "10px",
                border: "none",
                background: merchant?.is_open
                  ? "var(--color-green-dim)"
                  : "var(--color-red-dim)",
                color: merchant?.is_open
                  ? "var(--color-green)"
                  : "var(--color-red)",
                fontSize: "11px",
                fontWeight: 800,
      ...
      <div style={{ padding: "16px", paddingBottom: "40px" }}>
        {error && (
          ...
          >
            <p
              style={{ fontSize: "12px", color: "var(--color-red)", margin: 0 }}
            >
              {error}
            </p>
          </div>
          ...
          <div>
            ...
            >
              {[
                {
                  label: "Calificación",
                  value: "★ " + (merchant.rating ?? 4.5).toFixed(1),
                  color: "var(--color-yellow)",
                },
                {
                  label: "Entrega",
                  value: merchant.delivery_time ?? "—",
                  color: "var(--text-primary)",
                },
                {
                  label: "Productos",
              ...
              >
                {[
                  ...
                  },
                  {
                    ...
                    val: editDesc,
                    set: setEditDesc,
                    ph: "Breve descripción de tu negocio",
                    disabled: !editandoInfo,
                  },
                  {
        ...
        {tab === "menu" && (
          <div>
            ...
            >
              ...
              <button
                onClick={function () {
                  setModalProd({ is_available: true });
                }}
                style={{
                  display: "flex",
                  alignItems: "center",
              ...
              >
                <Plus style={{ width: "14px", height: "14px" }} /> Nuevo
                producto
              </button>
            </div>
            ...
            {categorias.map(function (cat) {
              const prods = productos.filter(function (p) {
                return (p.category ?? "General") === cat;
              });
              ...
              return (
                <div key={cat} style={{ marginBottom: "10px" }}>
                  <button
                    onClick={function () {
                      setCatAbierta(abierta ? null : cat);
                    }}
                    style={{
                      width: "100%",
                      display: "flex",
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              background: "var(--bg-card)",
                              border: "1px solid var(--border-subtle)",
                              opacity: prod.is_available ? 1 : 0.5,
                            }}
                          >
                            <div
                              style={{
          ...
          >
            ...
            >
              {[
                {
                  label: "Nombre *",
                  val: modalProducto.name ?? "",
                  key: "name",
                  ph: "Nombre del producto",
                },
                {
                  label: "Descripción",
                  val: modalProducto.description ?? "",
                  key: "description",
                  ph: "Descripción breve",
                },
                {
                  label: "Categoría",
                  val: modalProducto.category ?? "",
                  key: "category",
                  ph: "Comidas, Bebidas, Extras...",
                },
                {
                  label: "URL de imagen",
                  val: modalProducto.image_url ?? "",
                  key: "image_url",
                  ph: "https://...",
                },
              ].map(function (f) {
              ...
              >
                <input
                  type="checkbox"
                  checked={modalProducto.is_available ?? true}
                  onChange={function (e) {
                    setModalProd(function (prev) {
                      return prev
    ...
    <div>
      ...
      {pedidos.slice(0, 10).map(function (p) {
        ...
        const colorEstatus: Record<string, string> = {
          pendiente: "var(--color-yellow)",
          en_camino: "var(--color-blue)",
          entregado: "var(--color-green)",
            ...
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              ...
              <span
                style={{
                  ...
                  color: colorEstatus[p.estatus] ?? "var(--text-muted)",
                  textTransform: "uppercase",
                }}
              >
                {p.estatus}
